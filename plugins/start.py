...
REPLY_ERROR = "Please reply to a message to broadcast."


@Bot.on_message(filters.command('users') & filters.private & filters.user(ADMINS))
async def get_users(client: Bot, message: Message):
    try:
        chat_info = await client.get_chat(chat_id=CHANNEL_ID)
        member_count = chat_info.members_count
        await message.reply(f"Total {member_count} users are in the channel.")
...
async def send_text(client: Bot, message: Message):
    if message.reply_to_message:
        try:
            chat_info = await client.get_chat(chat_id=CHANNEL_ID)
            member_count = chat_info.members_count
            pls_wait = await message.reply(f"<i>Broadcasting Message to {member_count} users.. This will Take Some Time</i>")
            ...
            async for member in client.iter_chat_members(chat_id=CHANNEL_ID):
                try:
                    await broadcast_msg.copy(member.user.id)
                    successful += 1
                except FloodWait as e:
                    await asyncio.sleep(e.x)
                    await broadcast_msg.copy(member.user.id)
